...
const PAYMENT_STORAGE_KEY = 'ai-company-analyzer-payment-data';
...
function PaymentGateway({ companyData, onPaymentComplete, onCancel, discountCode }) {
  ...
  const savedPaymentData = loadSavedPaymentData();
  
  const [selectedPlan, setSelectedPlan] = useState(savedPaymentData?.selectedPlan || 'premium');
  const [isProcessing, setIsProcessing] = useState(false);
  const [error, setError] = useState(null);
  const [paymentMethod, setPaymentMethod] = useState(savedPaymentData?.paymentMethod || 'card');
  const [email, setEmail] = useState(companyData.email || savedPaymentData?.email || '');
  const [fullName, setFullName] = useState(companyData.fullName || savedPaymentData?.fullName || '');
  const [cardDetails, setCardDetails] = useState(savedPaymentData?.cardDetails || {
    cardNumber: '',
    expiry: '',
    cvc: '',
    name: ''
  ...
  }, [selectedPlan, paymentMethod, email, fullName, cardDetails]);
  ...
  const handlePayment = async (e) => {
    ...
    try {
      // Demo mode: Simplified validation - just require email
      if (!email.trim()) {
      ...
      // For demo: Auto-fill name if not provided
      const finalName = fullName.trim() || 'Demo User';

      // Process payment (demo mode - always succeeds)
      const paymentData = {
        plan: selectedPlan,
        amount: finalPrice,
        paymentMethod: paymentMethod || 'card',
        cardDetails: paymentMethod === 'card' ? {
          cardNumber: cardDetails.cardNumber || '4242 4242 4242 4242',
          expiry: cardDetails.expiry || '12/25',
          cvc: cardDetails.cvc || '123',
          name: cardDetails.name || finalName
        } : null,
        companyData: {
          ...companyData,
          email: email,
      ...
      const result = {
        success: true,
        transactionId: `demo-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`
      };
      
      if (result.success) {
        // Clear saved payment data on successful payment
        try {
            ...
            <div className="space-y-4 mb-6">
              <div className="bg-blue-50 border-l-4 border-blue-500 p-3 rounded-r-lg mb-4">
                <p className="text-sm text-blue-800">
                  <strong>Demo Mode:</strong> Card details are optional. Payment will succeed automatically.
                </p>
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Card Number <span className="text-gray-400 text-xs">(optional)</span>
                </label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">
                    CVC <span className="text-gray-400 text-xs">(optional)</span>
                  </label>
                  <input
                    type="text"
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">
                  Cardholder Name <span className="text-gray-400 text-xs">(optional)</span>
                </label>
                <input
                  type="text"
